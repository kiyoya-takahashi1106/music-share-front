...
export const deleteRoom = async (roomId) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/room/delete`, { room_id: roomId });
    return response.data;
  } catch (error) {
...
// 現在の曲を変更する関数
export const updateNowPlaying = async (roomId, playingSongName) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/room/next-song`, {
      room_id: roomId,
      playing_song_name: playingSongName,
    });
    return response.data;
  } catch (error) {
    console.error('Error updating now playing song:', error);
    throw error;
  }
};
...
export const setNextPlaylist = async (roomId, playingPlaylistName) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/room/next-playlist`, {
      room_id: roomId,
      playing_playlist_name: playingPlaylistName,
    });
    return response.data;
  } catch (error) {
